...
const DiscussionDetailPage = () => {
    ...
    const { data: discussion, error: discussionError, isLoading: discussionLoading, mutate: discussionMutate } = useSWR(`v1/discussions/${id}`, url => axiosInstance.get(url).then(res => res.data))

    const { data: replies, error: repliesError, isLoading: repliesLoading, mutate: repliesMutate } = useSWR(`v1/replies?discussion=${id}`, url => axiosInstance.get(url).then(res => res.data))
